...
class Solution{
    ...
    String calc_Sum(int a[], int n, int b[], int m)
    {
        ...
        for (int num : sum) {
            if (num != 0) {
                leadingZero = false; // Reset flag if non-zero digit encountered
            }
            if (!leadingZero) {
                ans.append(num); // Append digit if not a leading zero
            }
        }
        
        if (ans.length() == 0) {
            ans.append('0'); // If result is zero, append a single '0'
        }
